...
// Validate and sanitize tournament_id
$tournament_id = isset($_GET['id']) ? trim($_GET['id']) : null;
if ($tournament_id === null) {
    die('<div class="container my-5"><div class="alert alert-danger text-center">No tournament ID provided. Please select a tournament from the dashboard.</div></div>');
}
$tournament_id = is_numeric($tournament_id) ? (int)$tournament_id : null;
if ($tournament_id === null) {
    die('<div class="container my-5"><div class="alert alert-danger text-center">Invalid tournament ID format. Use a number (e.g., ?id=1).</div></div>');
}
...
if (!$tournament) {
    die('<div class="container my-5"><div class="alert alert-danger text-center">Tournament not found. Create a tournament or check the ID (e.g., ?id=1).</div></div>');
}

// Fetch organizer name
$organizer_stmt = $pdo->prepare("SELECT username FROM users WHERE user_id = ?");
$organizer_stmt->execute([$tournament['organizer_id']]);
$organizer_name = $organizer_stmt->fetchColumn() ?: 'Unknown';

// Fetch matches for this tournament
$stmt = $pdo->prepare("SELECT m.match_id, t1.name AS team1, t2.name AS team2, m.score_team1, m.score_team2, m.status, r.winner_id, m.team1_id, m.team2_id 
                       FROM matches m 
...
foreach ($matches as $match) {
    $team1 = $match['team1'];
    $team2 = $match['team2'];
    $team1_score = $match['score_team1'] ?? 0;
    $team2_score = $match['score_team2'] ?? 0;
...
uasort($standings, fn($a, $b) => $b['points'] <=> $a['points']);
?>
                    ...
                    <tbody>
                        <?php foreach ($matches as $match): ?>
                            <tr>
                                ...
                                <td><?php echo ($match['score_team1'] ?? 0) . ' - ' . ($match['score_team2'] ?? 0); ?></td>
